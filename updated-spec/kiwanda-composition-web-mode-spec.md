...
**Status:** Built and validated — pipeline (Inventory → Compress→brand.md → Expand→vault → Compose → Pages) runs headless end to end, producing Kladi, Jikoni, and Rembo as visually distinct sites from the same generator. Filter and attribute-selector both working. Remaining items below are fixes and small additions, not open architecture questions.
...
- **Product detail** — image + text-zone shape (adapted from `hero-photo-stat` / `list-with-mockup`). One attribute-selector instance per attribute flagged `selectable`, placed above the CTA set; both CTAs stay disabled until every selectable attribute has a chosen value. Attributes not flagged selectable render as plain descriptive text. Copy handling inverts from posters — variable-length seller-authored descriptions need graceful overflow (truncate/expand), not a hard cap
...
- **Filter control** — reads only attributes flagged `filterable` on the product entity, not the full attribute set. Categories are just another filterable attribute, not a route. Handles zero-result filter combinations
- **Cart overlay** — see below
...
- `selectable` — true/false: buyer must choose a value on the product page before the CTA set is enabled

The two flags are independent, not alternatives — an attribute can be both, one, or neither.

## Product-page CTA set

Two CTAs, same underlying mechanism:
- **Add to Cart** — adds the product (with selected attribute values) to cart state
- **Buy Now** — does exactly what Add to Cart does, then additionally auto-opens the cart overlay immediately, surfacing the pre-checkout form. Not a separate WhatsApp-direct shortcut — there is only one checkout path, Buy Now is just a faster route into it.
...
- **Cart state**: client-side, holds added products, quantities, and any selected attribute values; persists across page navigation within a session (e.g. localStorage)
...
## Token safety (new — fix surfaced by Kladi build)

Brand.md's color-token extraction currently has no contrast check between extracted text/background pairs. Kladi shipped with button text that's unreadable against its background. Add a contrast-ratio check (WCAG-style, text vs. its background token) at token-generation time, with a safe fallback — force a neutral text color when the brand pair fails the check. This is a system-level fix, not a one-off Kladi patch: any seller's auto-extracted palette can produce the same failure.
...
Must run headless. Duka Bee's base tier is zero-touch, so web mode has to generate valid pages without a human in the editor UI per page.
...
- Saved carts across sessions/devices

## Acceptance criteria

- [x] Header/footer sections render consistently across all pages, sourced from brand.md tokens; header includes a functioning cart icon with item count
- [x] Product grid handles 1 to N products with responsive reflow, no fixed slot-count assumption
- [x] Product detail page renders one attribute-selector per selectable attribute; non-selectable attributes render as plain text; variable-length descriptions don't break layout
- [x] Filter control reads only filterable-flagged attributes and handles the zero-result state
- [x] Cart overlay opens from the header on any page, lists cart contents including selected attribute values, and its checkout button opens the pre-checkout form
- [x] Full pipeline (vault → homepage + shop + N product pages) runs headless, no editor UI step required
- [ ] Buy Now added alongside Add to Cart, shares mechanism, auto-opens cart overlay
- [ ] Contrast-safety check added to the token pipeline; Kladi's button-text issue fixed as the first case
